...
- `WORKER_POLL_INTERVAL_MS` -- runner loop interval.
...
- Deploy `apps/web` and `apps/worker` (runner) separately.
- Ensure the runner has access to `RUNS_DIR` and `UPLOADS_DIR`.
- The runner should be deployed as a private service (no public URL).
